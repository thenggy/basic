...
class AStarActionServer(Node):
    ...
    def pose_estimate_callback(self, initial_pose):
        ...
        self.get_logger().info(f"Start position not ready. :{self.start}")
    ...
    def get_path(self):
        ...
        if path.size > 0:
            ...
            self.visualize_path()
            self.follow_path()  # Call follow_path now that path is ready
        else:
            self.get_logger().info("No path found")
            self.path_cart = []
        # else:
    ...
    def follow_path(self):
        ...
        if not self.start_goal or  len(self.path_cart) == 0:
            return  # Exit if no goal set or path is not ready
        ...
        self.stop_robot()  # Stop the robot after completing the path 
