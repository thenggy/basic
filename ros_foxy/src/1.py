...
class PathPublisherNode(Node):
    ...
    #         # Clear existing data in ox and oy lists
    #         self.map_ox = []
    #         self.map_oy = []
    #         for i in range(self.occupancy_map.shape[0]):
    #             for j in range(self.occupancy_map.shape[1]):
    #                 # Check if the cell is occupied (you may need to adjust this condition based on your occupancy map)
    #                 if self.occupancy_map[i, j] > 40 :
    #                     # Convert grid indices to x and y coordinates
    #                     x = j * self.map_res + self.origin[0]
    #                     y = i * self.map_res + self.origin[1]

    #                     # Append to ox and oy lists
    #                     self.map_ox.append(x)
    #                     self.map_oy.append(y)
    #         # self.ox, self.oy = remove_close_obstacles(self.map_ox, self.map_oy, 0.1)
    #         self.ox = self.map_ox
    #         self.oy = self.map_oy
